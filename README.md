...
8. **계통 분류별 탐색 디렉토리 자동 생성 (`taxonomy/`)**
   - 다운로드 완료 시, 각 유전체의 문(Phylum) ➔ 강(Class) 분류 수준에 맞게 자동으로 `taxonomy/<Phylum>/<Class>/` 폴더 트리를 빌드합니다.
   - 해당 강(Class) 폴더 하위에 실제 유전체 원본 폴더(`all_genomes/...`)를 가리키는 **디렉토리 레벨 상대 경로 심볼릭 링크**를 자동 생성하여, 파일 브라우저에서 계통학적으로 데이터를 빠르게 훑어보고 개수를 직관적으로 카운트할 수 있습니다.
